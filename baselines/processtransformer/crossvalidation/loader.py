...
class LogsDataLoader:
    ...
    # Achtung! the following method is adjusted, since in _remaining_time_helper_func in processor.py
    # we excluded the last prefix which is essentially the whole trace: we need to add one to have the
    # correct number for max_case_length in the training dataset.
    ...
    # Achtung! fold is added to the signature of this method to handle K-fold cross validation
    def load_data(self, task, fold):
        if task not in (Task.NEXT_ACTIVITY,
            Task.NEXT_TIME,
            Task.REMAINING_TIME):
            raise ValueError("Invalid task.")
        # Achtung! following lines are commented/added to handle K-fold cross validation
        #train_df = pd.read_csv(f"{self._dir_path}/{task.value}_train.csv")
        train_df = pd.read_csv(f"{self._dir_path}/{Task.REMAINING_TIME.value}_train_fold_{fold}.csv")
        test_df = pd.read_csv(f"{self._dir_path}/{Task.REMAINING_TIME.value}_test_fold_{fold}.csv")
