...
# Achtung!the following lines are added to handle different random seed
random_seed = 89  # Replace with the desired seed value
...
if __name__ == "__main__":
    ...
    # Achtung! this loop and some other parts are changed to handle cross-fold validation
    num_folds = 5
    for fold in range(num_folds):
        model_path = f"{model_path}/fold_{fold}" # to save seperate checkpoint for each fold.
        print('Fold:', fold)
        (train_df, test_df, x_word_dict, y_word_dict, max_case_length,
         vocab_size, num_output) = data_loader.load_data(args.task, fold)
        ...
        transformer_model = transformer.get_remaining_time_model(max_case_length=max_case_length,
        ...
        results_df.to_csv(result_path+f"_remaining_time_train_fold_{fold}.csv", index=False)
