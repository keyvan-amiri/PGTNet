...
class LogsDataProcessor:
    ...
    def _load_df(self, sort_temporally = False):
        ...
        # Achtung! timestamp format is revised to be able to handle other event logs.
    ...
    def _remaining_time_helper_func(self, df):
        ...
        for _, case in enumerate(unique_cases):
            ...
            latest_diff = datetime.timedelta()
            recent_diff = datetime.timedelta()
            # Achtung! the followings are adjusted: remove prefixes of length 1,n (fair comparison)
    ...
    def _process_remaining_time(self, df, train_list, test_list, fold):
        df_split = np.array_split(df, self._pool)
        with Pool(processes=self._pool) as pool:
        ...
        # Achtung! in the following to lines {fold} is added to file name to handle K-fold cross validation
        train_remaining_time.to_csv(f"{self._dir_path}/{Task.REMAINING_TIME.value}_train_fold_{fold}.csv", index = False)
        test_remaining_time.to_csv(f"{self._dir_path}/{Task.REMAINING_TIME.value}_test_fold_{fold}.csv", index = False)
    ...
    def process_logs(self, task, 
        ...
        # Achtung! the following two lines are commented to handle K-fold cross validation
        #train_list = df["case:concept:name"].unique()[:train_test_ratio]
        #test_list = df["case:concept:name"].unique()[train_test_ratio:]
        # Achtung! the following two line as well as the for loop are added for the same purpose.
        num_folds = 5
        kf = KFold(n_splits=num_folds, shuffle=True, random_state=42)
        for fold, (train_idx, test_idx) in enumerate(kf.split(df["case:concept:name"].unique())):
            train_list = df["case:concept:name"].unique()[train_idx]
            test_list = df["case:concept:name"].unique()[test_idx]
            if task == Task.NEXT_ACTIVITY:
                ...
                self._process_remaining_time(df, train_list, test_list, fold)
            else:
