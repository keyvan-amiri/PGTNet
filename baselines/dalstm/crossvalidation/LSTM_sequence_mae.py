...
import pandas as pd # achtung! additional import for earliness analysis
...
# Achtung!to properly handle prefix lengths, this part is modified.
(_, _, _), values = load_dataset(args.full_dataset)
...
(X_train, y_train, train_original_lengths), _ = load_dataset(os.path.join(dataset_directory, "train_" + dataset_name), values)
(X_val, y_val, val_original_lengths), _ = load_dataset(os.path.join(dataset_directory, "val_" + dataset_name), values)
(X_test, y_test, test_original_lengths), _ = load_dataset(os.path.join(dataset_directory, "test_" + dataset_name), values)
...
if args.test:
    ...
    # Achtung! the following code is added to compute MAE for each test example
    # this result is then saved in as a csv file
    example_results = []  # Store per-example results for CSV output
    # Make predictions on the test set
    y_pred = testmodel.predict(X_test)
    for i in range(len(X_test)):
        actual_length = test_original_lengths[i]
        example_mae_days = np.abs((y_pred[i] - y_test[i]) / (24.0 * 3600))  # Convert seconds to days
        example_results.append((actual_length, example_mae_days))  # Append to per-example results
    # Save per-example results to a CSV file (new addition)
    csv_filename = os.path.join("results", dataset_name + "_per_example.csv")
    pd.DataFrame(example_results, columns=["pl", "absolute_error"]).to_csv(csv_filename, index=False)